...
逐张筛选路径复核；无法精确复现目标照片集合时不会给出可应用的阈值。评估不会自动
开始标定；「使用建议值」只填写阈值（“不剔除”会清空阈值），仍需用户点击「运行标定」。

这只是当前相机模型与当前照片集上的比较建议，不是准确度保证，也不是标定合格判据。
它与上面的 MAD 首轮统计离群检查相互独立。LOOCV 需要串行重标多次，界面会显示进度，
可在单次 OpenCV 标定结束后取消；取消不会改动已保存的标定结果。
