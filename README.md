...
程序会根据首轮逐视图 RMS 计算一个可选的推荐阈值：用中位数（median）和
MAD（各误差与中位数绝对差的中位数）估计稳健离散程度，按
`median + 3 × 1.4826 × MAD` 识别明显高误差视图，再把建议阈值放在最大正常值与
最小异常值之间。样本不足、MAD 退化或没有明显异常时，不会强行给出数值建议。

推荐值只是辅助建议，不会自动填入阈值，也不是标定质量合格标准。点击「使用推荐值」
只会把数值放进阈值输入框；只有再点击「运行标定」才会开始迭代重标定。运行时若阈值
相较最近一次结果有变化，界面会自动重新标定；勾选「强制重新标定」可在阈值未变时也重算。
阈值预览始终
基于第一轮误差，界面会显示：
...
尺度变化和参数稳定性提示，并将最需要改善的方向转成补拍建议。覆盖热力图按 8×6 个归一化
...
| 参数稳定性 | 参考 OpenCV 返回的 `fx/fy/cx/cy` 标准差相对焦距或图像宽高的比例；均 ≤2% 为良好，均 ≤5% 为一般，否则较弱 |

参数标准差仅是**当前模型和当前数据下的估计不确定性参考**，不能单独证明模型正确或标定可靠。
位置重复和其他低信息量视图只会触发建议，不会被自动删除；已有的高重投影误差筛选是另一项
独立功能。建议结合热力图、逐照片数据、重投影误差、去畸变预览和实际成像综合判断，并优先按
提示补拍缺失位置、边缘、姿态或距离的照片。
