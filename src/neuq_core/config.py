"""配置与工程状态：标定板规格、素材来源依据和可恢复的导入事务。

这里保存的不只是“偏好设置”，还有日后解释结果所需的来源证明（provenance）：这套
素材按哪块棋盘分过类、哪次导入生成了它、一次覆盖换装是否完成。算法结果如果脱离这些
前置事实，即使 JSON 字段齐全，也可能已经不适用于当前工程。

状态检查采取 fail closed：遇到 ``pending``、``.old`` 或说不清来源的旧素材时，不猜测
哪一份是对的。原始照片可能不可再生，相比留下可人工处理的残留，自动删除唯一副本的
代价更大。目录 rename/rollback 的机械动作在 :mod:`neuq_core.fs_transaction`，本模块
负责赋予这些状态以工程含义。
"""
...
class MaterialImportTransaction:
    ...
    ``pending`` 是写给“下次启动”的路标：如果进程在任意一步中断，后来的代码会知道
    正式目录可能处于中间态，而不会把它误当成一套完整素材继续标定。``material_set``
    则是已提交素材的来源证明，只有最终配置写成功后才能更新。
