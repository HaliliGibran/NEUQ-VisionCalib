...
车载相机拍到的赛道既有镜头畸变，又有透视效果，直接拿原图做距离、横向偏差和曲率计算会很麻烦。
NEUQ-VisionCalib 把**相机标定 → 去畸变 → 地面逆透视 → 板端查找表导出**串成一套流程，
把地面区域转换成更适合传统视觉处理的俯视图。

**输入：** 棋盘照片 + 地面标定照片

**输出：** 相机参数、逆透视矩阵，以及可直接给嵌入式端使用的去畸变 / 去畸变+逆透视查找表

---

各位 NEUQ 的智能车 er，这套工具基本覆盖了传统视觉方向里常用的相机标定与地面投影参数。
...
| 2  | **运行相机标定**                                   | 导入后会自动先做一轮无剔除标定；查看第一轮误差分布后，可选阈值并运行迭代重标定 |
...
标定页可以选择**标准镜头（针孔模型）**或**鱼眼镜头（Fisheye 模型）**。普通摄像头和常见广角镜头
优先使用标准模型；镜头明确是鱼眼，或边缘形变、压缩特别明显时再选鱼眼。不确定时先用标准模型。
两种模型的畸变参数定义不同，不能互换；切换模型后需要重新标定。没有 `camera_model` 字段的旧
`calib.json` 按标准模型读取。相机模型也参与标定基准哈希，因此切换后的旧 IPM/LUT 不会被误复用。
...
| `D`    | 畸变参数；标准模型为 `[k1, k2, p1, p2, k3]`，鱼眼模型为 4 个鱼眼系数 |
...
界面上的柱状图给出**第一轮、尚未剔除任何照片时**每张照片的 RMS，并按误差从高到低排列。
导入素材后已经自动完成这一轮无剔除标定，因此可以直接查看误差分布，不必先手动盲跑一次。

### 离群照片提醒与手动筛选

程序会根据第一轮逐张误差做离群检查：用中位数（median）和
MAD（各误差与中位数绝对差的中位数）估计稳健离散程度，按
`median + 3 × 1.4826 × MAD` 标记误差明显偏大的照片。若检测到这样的照片，界面会显示用于区分
离群项的分界值；它只解释误差分布，不是“筛到这里能让标定最好”的推荐阈值。
照片太少、误差分布太集中或没有明显离群项时，不会给出分界值。

这项检查只做提醒，不会自动删照片，也不判断是否应该删除这些照片。**没发现离群照片，
...
> 按这个上限会先筛掉 N / M 张；重新标定后实际数量可能变。

每轮都会用当前保留的照片完整重标定；若仍有照片超过上限，只移除当前误差最大的一张，
再重新标定并判断。程序至少保留 3 张；如果达到这个下限时仍有照片超限，会停止
继续剔除并明确提示原因。标定结果会列出最后使用的张数、剔除张数与最终 RMS。

### 照片筛选建议（留一验证评估）

「开始评估」会比较全部保留和不同剔除程度下的结果。程序每次留出一张照片，用其余照片生成
一条逐张剔除路径，再检查留出照片在路径上各个标定结果下的误差。留出照片不会参与内参、畸变拟合
或删帧决策；计算误差时，程序会固定候选模型的内参和畸变系数，只为留出照片单独求外参。

每个留一折只计算一条剔除路径，不同保留张数直接复用路径上已经得到的标定结果，不会为每个候选
重新从头标定。各个留一折可以并行运行。评估还会检查画面覆盖、边缘覆盖、姿态和尺度变化，避免
为了降低误差而明显牺牲这些信息。

最后使用 one-SE 规则选方案：如果几个方案的验证误差差不多，就优先保留更多照片。程序还会在完整
数据上复核候选阈值，确认所选阈值能得到预期的照片集合；对不上就不会提供可直接使用的阈值。
「使用建议」只会填写阈值（建议不剔除时会清空阈值），不会自动开始标定，仍需点击「运行标定」。

这项评估和上面的离群提醒互不依赖，是额外的筛选参考，不是标定精度证明。照片较多时可能
需要运行几分钟；评估期间仍可以继续做逆透视选图和预览。取消评估不会改动已保存的标定结果。
