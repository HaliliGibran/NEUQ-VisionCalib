...
「评估筛选建议」会比较不剔除以及保留 4 张以上的不同数量候选。每个留一折只用其余照片
拟合一次逐张剔除路径，不同保留数量候选复用路径上对应的模型。留出照片不参与拟合或删帧决策，
只在固定内参和畸变系数下估计自身外参并计算重投影 RMS。评估同时要求画面覆盖、边缘覆盖、
姿态变化和尺度变化的现有诊断等级不低于该折的不剔除基线。

候选根据留一验证误差（LOOCV）比较，并采用一标准误（one-SE）规则：多个方案的验证表现足够接近时，
优先保留更多照片，并始终把“不剔除”作为基线。选出的保留数量还要在完整数据上反推出阈值，并通过正式
逐张筛选路径复核；无法精确复现目标照片集合时不会给出可应用的阈值。独立留一折由有界进程池并行计算。
评估不会自动开始标定；「使用建议值」只填写阈值（“不剔除”会清空阈值），仍需用户点击「运行标定」。

这只是当前相机模型与当前照片集上的比较建议，不是准确度保证，也不是标定合格判据。
它与上面的 MAD 首轮统计离群检查相互独立。界面会显示评估进度；照片读取完成后，后台评估
不会长期阻塞逆透视选图和预览等操作。根据照片数量和电脑性能，完整 LOOCV 评估可能需要数分钟。
取消会停止尚未开始的留一折，并在各进程当前单次 OpenCV 标定结束后退出；取消不会改动已保存的标定结果。
