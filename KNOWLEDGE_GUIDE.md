...
本工具允许选择标准（针孔）或鱼眼相机模型。两者都使用内参矩阵 `K` 描述焦距和主点，但成像投影
及畸变模型不同。普通相机标定默认使用标准模型；以下针孔关系和第 4 节的五参数公式只适用于该模型。
...
鱼眼模型不是把标准模型多加几个系数，而是另一套投影。它先取 `theta = atan(r)`，再用四个
鱼眼畸变系数计算 `theta_d = theta · (1 + k1·theta² + k2·theta⁴ + k3·theta⁶ + k4·theta⁸)`，
最后按 `theta_d / r` 缩放归一化坐标并通过 K 映射到像素。因而标准模型和鱼眼模型的 D 长度、
系数含义与尺度都不同；不能仅凭某个畸变系数大小直接比较两种模型。`calib.json` 会记录
`camera_model`，模型选择也会进入标定基准哈希，确保去畸变、IPM、LUT 与验证沿用同一模型。
