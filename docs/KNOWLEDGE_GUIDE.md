...
| 逆透视参考坐标系 | 从标定矩形坐标系平移、旋转后的地面 | cm，原点取去畸变图底边中点对应的地面点 |
| BirdView 像素 | 最终俯视画布上的点 | px，x 向右、y 向下 |

“标定矩形坐标系”和“逆透视参考坐标系”尤其容易混。前者的原点只是实测矩形中心，
方便四点建立公制关系；后者才是最终构图用的参考原点。本项目把去畸变图**底边中点**
投到地面得到这个参考点。它不是相机光心，也不自动等于车头或保险杠位置。
...
这也解释了为什么 calibration basis 指纹必须包含 Knew：逆透视四点是在 Knew 定义的
...
- H：Knew 去畸变像素 → 最终 BirdView，单位从 px 变为输出 px。
...
标定矩形中心 ≠ 逆透视参考原点。前者随你把标定矩形摆在哪里而变；后者固定对应当前
画面底边中点的地面射线。把二者混为一谈，anchor 的物理含义就会漂。

### 9.3 R(heading)：绕参考原点调朝向

先平移、后旋转非常重要。写成 R · T(-ref)，点先被移到参考原点，再绕 (0,0) 旋转。
如果顺序反过来，reference_origin 会先绕标定矩形中心旋转，之后再平移，最终不再落到
anchor。由于图像 y 向下，本项目里的正角在屏幕上看是顺时针。

### 9.4 S(scale)：把厘米变成像素

scale 的单位是 px/cm。x、y 使用相同倍率，才保留公制纵横比。地面上 45 cm 的正方形
在 BirdView 里仍应是正方形。

### 9.5 T(anchor)：把参考原点摆到画布上

anchor_x、anchor_y 是 0 到 1 的归一化位置，实际像素为：

~~~text
anchor_px = (anchor_x · (W-1), anchor_y · (H-1))
~~~

因此最终 H 应把去畸变图底边中点准确送到 anchor_px。它是一条很有价值的验收不变量。
...
画布底部，并求能完整容纳这个窗口的最大 scale。

为什么 fit_bottom_aligned 可以闭式求解，而不用一点点试 scale？对地面点 q：

~~~text
output = anchor + scale · q
~~~

在 anchor_x 和贴底位置确定后，上、左、右边界对 scale 都只给出一次不等式上限。
把“纵向装得下、左侧装得下、右侧装得下”的上限取最小值，就是最大可行 scale。

full_fov_fit_scale 的含义不同：它是“若想把整个数学有效视野都装下，需要多小的
scale”。它是诊断量，不是用户比例尺的上限。目标窗口更小，所以正常 scale 完全可能
大于它。
...
假设地面上一条真实边量得 45 cm，它在最终 BirdView 中应该占 215.1 px，那么：
...
含义是 BirdView 中每前进 4.78 个像素，地面距离前进 1 cm。反过来：
...
LUT 是 **Look-Up Table，查找表**。若嵌入式端每帧都对每个像素计算 r²、r⁴、r⁶、
...
| undistort_ipm/reverse | BirdView 输出像素 | 原始畸变图采样坐标 |
| undistort_ipm/forward | 原始畸变图像素 | BirdView 落点 |

如果只记住一句话：

> reverse 表是“输出图这个像素，要去源图哪里取颜色”。

复合 reverse LUT 尤其重要。对 BirdView 的每个输出像素，它先用 H⁻¹ 回到 Knew
...
1280×720 → 320×240，x 缩 4 倍、y 缩 3 倍。一个 BirdView 正方形会变成长方形，
角度、曲率、横向距离全部失真。后者不是“少量画质损失”，而是公制几何被破坏。

要求整数倍还让网格中心对应关系、内存布局和 C 端倍率都更明确。非整数比例并非数学上
...
X/Y 必须同时无效。若 X 是 sentinel、Y 却合法，C 端会拼出一个不存在的二维点。
...
想象源图远处只有一条 2 px 宽的赛道线。逆透视把远处拉开后，它也许要占 BirdView
...
若 J 的局部放大很大，源图 0.2 px 的坐标误差，到了 BirdView 可能变成几 px。
...
采样信息。** 改善它通常要从相机俯角、分辨率、焦距、目标窗口和 scale 的取舍入手。
...
> - [src/neuq_vision_calib.py](../src/neuq_vision_calib.py) 的
>   full_fov_fit_scale 与目标窗口布局，帮助识别过度拉伸
...
1. 标定矩形两条边长度分别等于实物宽/高 × scale；
2. 邻边仍垂直；
3. 上边方向等于 heading（按 180° 周期比较）；
4. 去畸变图底边中点最终落到 anchor。

**为什么这样验：** heading 不为 0 时，矩形本来就不轴对齐，拿“边必须水平/竖直”
作判据会误报。长度、垂直、朝向、参考原点对任意旋转都成立。

**失败通常意味着：** H 的组合顺序、四角对应、物理尺寸、比例尺、heading 或参考原点
语义有误。
...
**为什么这样验：** BirdView 的公制尺度应各向同性；320×240 不能冒充 1280×720 的
等比小图，320×180 才可以。
...
**为什么这样验：** 这是独立实现的 oracle，可以发现 reverse 方向写反、Knew 用错、
...
**验什么：** 四组表的 X/Y 无效掩码是否同步，解码后无效值是否统一为 -1。

**为什么这样验：** 一个二维采样点必须两个分量一起有效或一起无效，否则 C 端会拼出
假坐标。

**失败通常意味着：** 定点哨兵还原、文本/二进制序列化或 X/Y 配对有问题。

这项只验证哨兵的**表示一致性**，不判断无效区域的几何形状或连通性。测试知道自己没验
什么，和知道自己验了什么同样重要。
...
- OpenCV 图像参考是**独立 oracle**，重点检验数学方向和结果图；
- 流水线重算是**交付忠实度 oracle**，重点检验正反两张表、重采样、量化和落盘。

两者不是重复，而是互补。前者独立但在降采样时无法逐像素同位比较；后者覆盖所有格式，
却与生产链共享数学 helper，不能单独证明算法正确。

### 19.9 数学正确之外：provenance 与 transaction

产物还必须回答“它是用哪一版输入生成的”。本项目把 K、D、Knew、图像尺寸等组成
calibration basis 指纹；产物记录这个指纹。当前 basis 变化后，旧四点、H 或 LUT 会被
判为 **stale（过期）**，程序宁可要求重做，也不猜它们“可能还能用”。来源图再记录
